...
import {
  useContentDetailStore,
  type ContentDetailResponse,
} from '@/stores/contentDetail';

const mockApi = api as unknown as { get: ReturnType<typeof vi.fn> };

const fixture: ContentDetailResponse = {
  content: {
    id: 1,
    title: '도깨비',
    subtitle: '쓸쓸하고 찬란하神',
...
describe('contentDetail store', () => {
  beforeEach(() => {
    setActivePinia(createPinia());
    mockApi.get.mockReset();
  });

  it('fetch happy path populates work/progress/spots and calls GET /api/contents/:id', async () => {
    mockApi.get.mockResolvedValueOnce({ data: fixture });
    const store = useContentDetailStore();
    await store.fetch(1);

    expect(store.content).toEqual(fixture.content);
    ...
    expect(url).toBe('/api/contents/1');
  });
