 ...
 * `place.scene_image_url` / `work_episode` / `scene_timestamp` / `scene_description` 4 컬럼이
...
public class LegacyPhotoSchemaMigration implements CommandLineRunner {
    ...
    /**
     * place.{scene_image_url, work_episode, scene_timestamp, scene_description} → place_scene_image 자식 행으로 이전.
    ...
    private void migratePlaceSceneImage() {
        ...
        if (hasImage) {
            ...
            int copied = em.createNativeQuery(
                    ...
                    "  (place_id, image_url, image_order_index, work_episode, scene_timestamp, scene_description, CREATE_DATE, UPDATE_DATE) " +
