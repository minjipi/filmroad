...
public interface ContentRepository extends JpaRepository<Content, Long> {
     ...
     */
    @Query("""
            SELECT w FROM Content w
            WHERE LOWER(w.title) LIKE LOWER(CONCAT('%', :q, '%'))
            ORDER BY w.title ASC, w.id ASC
            """)
    List<Content> searchByTitle(@Param("q") String q, Pageable pageable);

    /**
     * 홈 "인기 작품" 섹션용 집계 쿼리. 작품별로 묶인 Place 의 trendingScore 합을 기준으로 정렬,
     * 동점은 placeCount DESC → title ASC 로 tie-break. Place 가 전혀 없는 Content 는 결과에서 제외됨
     * (홈 표면에 띄울 콘텐츠가 없으므로 의도된 동작).
     *
     * 반환: `[[Content w, Long placeCount, Long trendingScoreSum], ...]`.
     */
    @Query("""
            SELECT w, COUNT(p), SUM(p.trendingScore)
            FROM Place p JOIN p.content w
            GROUP BY w
