...
class SearchControllerTest {
    ...
    @Test
    @DisplayName("GET /api/search?q=이태원 → works + places 섹션에 매칭 반환")
    void search_byTerm_returnsHitsInBothSections() throws Exception {
        mockMvc.perform(get("/api/search").param("q", "이태원"))
                ...
                // 시드상 work 2 의 title 이 "이태원 클라쓰" 라서 works 섹션에 포함돼야 한다.
                .andExpect(jsonPath("$.results.works[*].id", hasItem(is(2))))
                .andExpect(jsonPath("$.results.works[*].placeCount",
                ...
                .andExpect(jsonPath("$.results.places[*].workId",
                        everyItem(greaterThan(0))));
    }

    @Test
    @DisplayName("GET /api/search?q=단밤 → 장소명(place.name) 단독 매칭 (work.title 엔 없음)")
    void search_byPlaceName_matchesOnPlaceNameOnly() throws Exception {
        // 시드상 "단밤 포차 (서울밤)" 만 name 에 '단밤' 포함. work.title/region 엔 없어서 works 는 비어야 함.
        mockMvc.perform(get("/api/search").param("q", "단밤"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.results.works", hasSize(0)))
                .andExpect(jsonPath("$.results.places", not(empty())))
    ...
    void search_byRegionLabel_matchesOnRegionOnly() throws Exception {
        // '강릉시' 는 place 10 의 regionLabel='강릉시 주문진읍' 에만 있음. works 에는 없어야 함.
    ...
    @Test
    @DisplayName("GET /api/search?q=도&limit=1 → works/places 각 섹션이 1건 이하")
    void search_limitApplied_cappedPerSection() throws Exception {
        mockMvc.perform(get("/api/search")
                ...
                .andExpect(jsonPath("$.results.works", hasSize(lessThanOrEqualTo(1))))
    ...
    void search_worksSection_placeCountIsAccurate() throws Exception {
        // 시드에 work_id=1('도깨비') 로 묶인 place: 10 주문진, 14 덕수궁 → placeCount=2.
        mockMvc.perform(get("/api/search").param("q", "도깨비"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.results.works[0].id", is(1)))
                .andExpect(jsonPath("$.results.works[0].title", is("도깨비")))
                .andExpect(jsonPath("$.results.works[0].placeCount", is(2)));
    }
}
